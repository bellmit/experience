...
import static com.excenergy.protocol.EnumTagMapper.*;
 ...
 * Date: 2013-09-16
 */
public class EnumTagHandler extends SimpleChannelInboundHandler<CommandData> {
    private static final Logger logger = LoggerFactory.getLogger(EnumTagHandler.class);
    private TagFactory tagFactory;

    public EnumTagHandler(TagFactory tagFactory) {
        this.tagFactory = tagFactory;
    }

    @Override
    protected void channelRead0(ChannelHandlerContext ctx, CommandData msg) throws Exception {
        if (msg.getCmdId() == CommandData.ENUM_TAG) {
            try {
                String para = decodeRequest(msg.getData());
                if (logger.isDebugEnabled()) {
                    logger.debug(String.format("Enum tag para:%s", para));
                }

                List<Tag> result;
                if (para.contains(SPLITTER)) {
                    String[] names = para.split(SPLITTER);
                    result = new ArrayList<>(names.length);
                    for (String name : names) {
                        Tag tag = tagFactory.get(name);
                        result.add(tag);
                    }
                } else {
                    result = tagFactory.enumTag(para);
                }
